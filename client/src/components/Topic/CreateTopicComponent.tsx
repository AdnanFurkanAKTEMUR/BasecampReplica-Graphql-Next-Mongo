...
import { CREATE_TOPIC } from '../../modules/resolvers/topicResolvers';
...
export default function CreateTopicComponent(props: any) {
  const { user }: any = useContext(AuthContext)
  const router = useRouter()

  const [createTopicFunc, { data: createData, loading: createLoading, error: createError }] = useMutation(CREATE_TOPIC)
  ...
  if (createLoading) return <div>Bekleyiniz</div>
  if (createError) return <div>Hata</div>
  return (
    <>
      {
        router.isReady ?
